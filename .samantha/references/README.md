...
| `coordination-protocol/` | Orchestrator–Implementer coordination — **M9 STAR** (**star mode**; formerly dual). **PROTOCOL 1.6.0**: canonical queue schema + writer/lint tooling (optional, per-repo pull-queue sites) on top of 1.5.0's protocol-version handshake (bootstrap exchange + re-arm-time staleness detection + self-upgrade pointer), 1.4.0's SSH message-signing (origin auth + tamper-evidence), and 1.3.0's project-scoped spoke awareness; skills `coordinate` + `coordinate-solo` + `coordinate-star`. Scripts share `PROTOCOL-VERSION` stamp with docs. |
| `coordination-protocol/README.md` | The full M9 protocol: topology, bootstrap, message grammar, the **6 disaster rules**, hub-mediated deploy windows, **addressed wake filter (1.2.1)**, **SSH message signing (1.4.0)**, **protocol-version handshake (1.5.0)**, **canonical queue schema + tooling (1.6.0)** |
