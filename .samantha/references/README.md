...
| `coordination-protocol/` | Orchestrator–Implementer coordination — **M9 STAR** (**star mode**; formerly dual). **PROTOCOL 1.3.0** project-scoped spoke awareness; skills `coordinate` + `coordinate-solo` + `coordinate-star`. Scripts share `PROTOCOL-VERSION` stamp with docs. |
| `coordination-protocol/README.md` | The full M9 protocol: topology, bootstrap, message grammar, the **6 disaster rules**, hub-mediated deploy windows, **addressed wake filter (1.2.1)** |
