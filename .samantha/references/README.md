...
| `coordination-protocol/` | Orchestrator–Implementer dual-instance coordination — **M9 STAR-topology**. **Solo installs:** copy the pack for reference but do not arm scripts. **Dual required:** `coord-monitor` · `coord-send` · `coord-status` · `heartbeat` · `bootstrap-identity` · `coordination-precommit-hook`. **Dual optional (PROTOCOL 1.1.0 robustness):** `coord-protocol-metrics` · `coord-session-healthcheck` · `coord-evidence-lint` · `coord-gate-audit`. Tombstones only under `retired/` (no executables). |
| `coordination-protocol/README.md` | The full M9 protocol: topology, bootstrap, message grammar, the **6 disaster rules**, hub-mediated deploy windows |
