...
EOF
  ...
  # stderr: human-readable guidance (does not pollute the capture).
  {
    ...
    echo "  ./coord-monitor.sh --identity $prov_id --dir $COORD_DIR   # arm via YOUR harness output→chat bridge (see CLAUDE.md: Arming the inbox)"
    echo ""
...
do_adopt() {
  ...
  # stderr: guidance for next steps.
  {
    ...
    echo "  ./coord-monitor.sh --identity $ASSIGNED --dir $COORD_DIR   # arm via YOUR harness output→chat bridge (see CLAUDE.md: Arming the inbox)"
    echo ""
