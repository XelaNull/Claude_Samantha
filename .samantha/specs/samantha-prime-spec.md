...
2. **THE THIRD MODE — background subagents (new default).** Samantha spawns Monk with `run_in_background` (and can spawn MULTIPLE for parallel zones — *she* spawns them, so depth-1 holds), stays LIVE while he runs an autonomous build-test-fix loop, and continues him via SendMessage; he returns only for decisions. In-session concurrency that **dominates foreground Mode A and pushes Mode B to the margins.** Hard limit: all background subagents share ONE context window + ONE compaction event — the bottleneck that finally earns Mode B. **Delivery caveat (2026-07-20, backported from Nebuspace):** background is ASYNC — a background worker's report reaches the parent ONLY on termination (completion notification) or an explicit `SendMessage` to `'main'`; its plain assistant text is invisible to the parent, so a bare idle ping is NOT a delivered report (general-purpose agents finish and go idle rather than terminate → report stuck in transcript). **When Samantha needs the result to PROCEED, spawn `run_in_background: false` (synchronous — report returns inline as the tool result); reserve background for fire-and-forget parallel work collected later via the completion notification.**
...
| 2026-06-27 | ✅ **Adopted OKF (Open Knowledge Format) as the canonical AI-knowledge format — replaces `.aispec` (the human's call):** Google's markdown-native OKF v0.1 (`.md` + YAML frontmatter — required `type` + recommended `title`/`description`/`resource`/`tags`/`timestamp`; structural body with `# Schema`/`# Examples`/`# Citations`; bundle-relative `/` links; reserved `index.md`/`log.md`; permissive "tolerate unknowns" consumption) becomes the format the Librarian carries + curates. Authored `references/okf-format.md` (replaces `references/aispec-format.md`, deleted). **Deeper scope (his choice):** re-architected the whole `canonical-docs-system` around OKF — recipe README, `SYSTEMS-hub-template` (now carries `type`), and the index generator (`INDEX.md`→`index.md`, reads `type`). Repointed every operational ref (README, CLAUDE.md, references/README, safety-carveouts, native memory). The §2/§5 Librarian **drive + curation model are unchanged** — only the carried *format* changes; supersedes the §2/§5 aispec-format decisions + the "codify `.aispec`" directive | OKF is **markdown-native**, so it *fits* the framework's single-source-of-truth / generate-from-canon posture instead of fighting it — a strictly better fit than `.aispec`, whose dense parallel hand-authored form was exactly what drifted. Historical §2/§5 aispec prose left intact as the record of that era; this entry carries the supersession (DOCS-WIN: current truth in the operational docs + this log) |
| 2026-08-12 | ✅ **Backported: canonical queue schema + tooling (PROTOCOL 1.6.0)** — a downstream deployment (Nebuspace) discovered its per-repo `queue-<repo>.md` files had drifted into 7-13 incompatible ad hoc table variants per file (no canonical writer/lint ever existed), including one row that stayed classified as buildable-now despite its own text reading "confirmed shipped... already done." Added `queue_schema.py` (canonical 6-column schema: `WO \| Priority \| Status \| Claimed-by \| Verified-against \| Description`, plus a `classify()` heuristic that cross-checks a row's declared Status against what its own content says), `queue-append.py` (the sanctioned writer — validates, refuses duplicate WO-ids, auto-routes cap overflow to backlog), `queue-lint.py` (read-only sweep) to `references/coordination-protocol/`. All three take `--dir`/`$COORD_DIR` and have zero hardcoded paths. Documented in README.md § Canonical queue schema + tooling; QUEUE-template.md cross-references it for sites using the per-repo pull-queue shape rather than the template's own single global push-queue schema (both are valid, don't mix them in one file) | This is the multi-project queue pattern (§ Multi-project coordination) needed a schema convention the base protocol never defined — nothing enforced row shape as a queue file grew over weeks of different agents hand-formatting rows. The two false-positive bugs found while *building* the classifier (WO-ids containing words like RETIRED/CLOSED self-triggering a DONE match; a bare GATED token matching "gated: no") are kept as comments in `queue_schema.py` itself so a future reimplementation doesn't reintroduce them |
